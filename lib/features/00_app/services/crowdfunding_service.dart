...
class CrowdfundingService {
  ...
  List<CrowdfundingEvent> generateFutureEvents({
    ...
    for (final entry in activeProjects.entries) {
      ...
      final asset = assets
          .where((a) => a.ticker == assetId || a.id == assetId)
          .firstOrNull;
      ...
      final buyTransactions = transactions
          .where((t) =>
              (t.assetTicker == assetId || t.assetTicker == asset.ticker) &&
      ...
      // Utilise désormais la durée maximale si disponible, sinon cible
      final durationMonths = asset.maxDuration ?? asset.targetDuration ?? 0;
      ...
      if (asset.repaymentType == RepaymentType.MonthlyInterest) {
        ...
        while (currentDate.isBefore(endDate) &&
            currentDate.isBefore(projectionEndDate)) {
          currentDate =
              currentDate.add(const Duration(days: 30)); // Approx month
        ...
        if (endDate.isAfter(now) && endDate.isBefore(projectionEndDate)) {
          final totalInterest =
              remainingCapital * yieldRate * (durationMonths / 12.0);
  ...
  List<CrowdfundingSimulationState> simulateCrowdfundingEvolution({
    ...
    final relevantTransactions = transactions.where((t) {
      ...
      if ((t.type == TransactionType.Deposit ||
          ...
          (t.notes.contains("Apport auto") ||
              t.notes.contains("Crowdfunding"))) {
        return true;
      }
  ...
  List<CrowdfundingProjection> generateProjections(List<Asset> assets) {
    ...
    for (final asset in assets) {
      ...
      final endDate =
          startDate.add(Duration(days: durationMonths * 30)); // Approx
      ...
      if (asset.repaymentType == RepaymentType.MonthlyInterest) {
        ...
        while (currentDate.isBefore(endDate)) {
          ...
          if (currentDate.isAfter(now)) {
            projections.add(CrowdfundingProjection(
              ...
              amount: monthlyInterest,
              type:
                  TransactionType.Interest, // Utilisation de Interest standard
            ));
          }
        }
        ...
        if (endDate.isAfter(now)) {
          ...
          final totalInterest =
              investedCapital * yieldRate * (durationMonths / 12.0);
